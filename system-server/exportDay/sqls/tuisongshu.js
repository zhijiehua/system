 ...
 * @Description: 获取推送数
 * @Author: huazj
 * @Date: 2024-04-13 19:17:42
 * @LastEditTime: 2024-04-13 19:35:00
 * @LastEditors: huazj
 */
const db = require('../../db/mysql');

const tuisongshu = async () => {
  ...
  let data = await db.query(
    ...
    left join (
      ...
      from t_device_group tdg left join t_device_group_glsb tdgg on tdg.id=tdgg.sbz_id where tdg.lx ='7' and notEmpty(parent_id)) a on twer.device_id =a.device_id
    where tdi.del_flag ='0' and tdi.device_type ='7' and twer.status ='0'
    and twer.wfxw in(?)
    and twer.hphm not in(?)
        and wfsj >= ?
        and wfsj  <= ?
